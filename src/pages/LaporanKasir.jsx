...
export default function LaporanKasir() {
    ...
    const fetchData = useCallback(async (isInitial = false, isLoadMore = false) => {
        if (!user) return;
        ...
        try {
            ...
            const from = isLoadMore ? transactions.length : 0;
            ...
            if (!error && data) {
                const newTransactions = isLoadMore ? [...transactions, ...data] : data;
                setTransactions(newTransactions);
                setHasMore(count > newTransactions.length);
                ...
                if (data.length > 0) {
                    ...
                    if (!itemsError && itemsData) {
                        setTransactionItems(prev => isLoadMore ? [...prev, ...itemsData] : itemsData);
                    }
                } else if (!isLoadMore) {
                    setTransactionItems([]);
                }

                // Only fetch summary notes on initial load or date change
                if (isInitial) {
                    try {
    ...
    }, [user, selectedDate, effectivePlan, isAdmin, t, transactions.length]);
                                        ...
                                        {transactions.length === 0 ? (
                                            ...
                                            transactions.map((tx) => (
                                                <tr key={tx.id} onClick={() => openTxDetail(tx)} className="hover:bg-slate-50 transition-colors cursor-pointer group">
                                                    ...
                                                    <td className="p-4 text-center">
                                                        <button onClick={(e) => { e.stopPropagation(); handleDeleteTransaction(tx); }} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={16} /></button>
                                                    </td>
                                                </tr>
                                            ))
                                        )}
                            ...
                            {hasMore && (
                                <div className="p-6 border-t border-slate-100 flex justify-center">
                                    <button 
                                        ...
                                        className="px-8 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold rounded-xl transition-all flex items-center gap-2 disabled:opacity-50"
                                    >
                                        {loadingMore ? (
